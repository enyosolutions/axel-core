...
class AxelAdminController {
  ...
  async listModels(req, res, next) {
    try {
      await execHook('axelModel', 'beforeApiFind', { request: req, sequelizeQuery: {} });
      const models = await AxelAdmin.serveModels();
      await execHook('axelModel', 'afterApiFind', { request: req, sequelizeQuery: {} });
      return res.json({
        body: models
      });
    } catch (err) {
  ...
  async getModel(req, resp) {
    ...
    try {
      await execHook('axelModel', 'beforeApiFindOne', { request: req, sequelizeQuery: {} });
