...
class AxelAdminPanelManager {
  ...
  init(app) {
    try {
      ...
      if (this.initialized) {
        console.warn('AxelAdminPanelManager already initialized. Please remove initialization from your bootstrap.js');
        return;
      }
      this.initialized = true;
      this.injectExpressRoutes(app);

      const axelModelConfig = loadSchemaModel(`${__dirname}/../models/schema/AxelModelConfig.js`);
      const axelModelFieldConfig = loadSchemaModel(`${__dirname}/../models/schema/AxelModelFieldConfig.js`);
 ...
 */
        ...
        case 'GET':
          ...
          AuthService.verify(req.headers.authorization, async (err, decryptedToken) => {
            ...
            try {
              ...
              debug('[ADMIN PANEL] WS client connected', socket.id, socket.user.username);
            } catch (err2) {
              cb(err2.message);
            }
          });
  ...
  wsProtectedRouting(socket) {
    ...
    socket.on('/admin-panel/restart-app', (req = { method: 'GET', query: {}, body: {} }, cb) => {
      if (typeof req === 'function') {
        cb = req;
      }
      switch (req.method) {
    ...
    /** Get models definition */
    socket.on('/admin-panel/auth', (req = { method: 'POST', query: {}, body: {} }, cb) => {
      if (typeof req === 'function') {
        cb = req;
      }
      switch (req.method) {
    ...
    /** Get models definition */
    socket.on('/admin-panel/config', (req = { method: 'GET', query: {}, body: {} }, cb) => {
      if (typeof req === 'function') {
        cb = req;
      }
      switch (req.method) {
